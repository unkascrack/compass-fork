    ...
    public void testCompassReindex() {
        ...
        compassTemplate.execute(new CompassCallbackWithoutResult() {
            protected void doInCompassWithoutResult(CompassSession session) throws CompassException {
                ...
                assertEquals("Harold", hits.resource(0).getValue(Petclinic.MetaData.FirstName.Name));
                if (hasClassMappings()) {
        ...
        compassTemplate.execute(new CompassCallbackWithoutResult() {
            protected void doInCompassWithoutResult(CompassSession session) throws CompassException {
                ...
                assertEquals(1, hits.getLength());
                assertEquals("Leo", hits.resource(0).getValue(Petclinic.MetaData.Name.Name));
                if (hasClassMappings()) {
        ...
        compassTemplate.execute(new CompassCallbackWithoutResult() {
            protected void doInCompassWithoutResult(CompassSession session) throws CompassException {
                ...
                assertEquals(1, hits.getLength());
                assertEquals("James", hits.resource(0).getValue(Petclinic.MetaData.FirstName.Name));
                if (hasClassMappings()) {
    ...
    public void testInsertOwnerCompassMirror() {
        ...
        compassTemplate.execute(new CompassCallbackWithoutResult() {
            protected void doInCompassWithoutResult(CompassSession session) throws CompassException {
                CompassHits hits = session.find("Schultz");
                assertEquals(1, hits.getLength());
                assertEquals("Schultz", hits.resource(0).getValue(Petclinic.MetaData.LastName.Name));
                if (hasClassMappings()) {
                    Owner owner = (Owner) hits.data(0);
    ...
    public void testInsertPetCompassMirror() {
        ...
        compassTemplate.execute(new CompassCallbackWithoutResult() {
            protected void doInCompassWithoutResult(CompassSession session) throws CompassException {
                CompassHits hits = session.find("bowser");
                assertEquals(1, hits.getLength());
                assertEquals("bowser", hits.resource(0).getValue(Petclinic.MetaData.Name.Name));
                if (hasClassMappings()) {
                    Pet pet = (Pet) hits.data(0);
